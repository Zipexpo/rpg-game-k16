...
(function() {
	...
	Window_SVBattler.prototype.refresh = function() {
	    ...
	    var actor = $gameParty.members()[this._index];
	    this.contents.clear();
	    var pic = ImageManager.loadSumRndmDdeMB(actor.actor().ams_bs_bust);
        var a = this._frame * (pic.width / 3);
        //var a = this._frame * (pic.width / 9);
        //this.contents.blt(pic, a, 0, pic.width / 9, pic.height / 6, eval(batX), eval(batY));
        this.contents.blt(pic, a, 0, pic.width /3 , pic.height / 1, eval(batX), eval(batY));
	};
